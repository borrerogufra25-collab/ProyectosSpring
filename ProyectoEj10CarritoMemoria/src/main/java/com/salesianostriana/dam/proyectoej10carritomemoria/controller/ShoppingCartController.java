...
public class ShoppingCartController {
    ...
    /*Versión 1 para agregar un producto al carrito: Lanzamos una excepción si no encontramos el producto y luego se pintará
     * una página de aviso*/
    ...
    /*Versión 2 para agregar producto al carrito: No se lanza excepción, solo se agrega si 
     * el objeto está dentro del Optional, si no está no hace nada, la 1 es mejor*/
    ...
    public String productoACarritoV2(@PathVariable("id") Long id, Model model) {
        ...
        productoService.findById(id).ifPresent(producto -> shoppingCartService.addProducto(producto));
        	    		 	
        return "redirect:/carrito";
    }
  
    
    /*Igualmente para el borrado de un producto del carrito
     * Versión 1 con excepción*/
    @GetMapping("/borrarProducto/{id}")
    public String removeProductFromCartV1(@PathVariable("id") Long id) {
        
        // Extrae el producto si existe; si no, lanza la excepción
        Producto producto = productoService.findById(id)
                .orElseThrow(() -> new NoSuchElementException("No se puede borrar: Producto no encontrado con ID " + id));
    ...
    public String removeProductFromCartV2(@PathVariable("id") Long id) {
        ...
        productoService.findById(id).ifPresent(producto -> shoppingCartService.removeProducto(producto));
        
        return "redirect:/carrito";
    }
    
    /*Mejor versión de borrado, usar en el servicio un borrar producto del carritoi por ID
     * */
 ...
 // Código ideal si usamos removeProductoPorId(Long id) que es más eficaz
    ...
    public String vaciarYNuevaCompra() {
        ...
        // 2. Si el carrito existe, usamos .clear() para vaciar todos los productos en memoria
        if (carrito != null) {
            // Nota: Si getProductsInCart() devuelve un mapa inmutable, 
            // deberás añadir un método específico en ShoppingCartService como "vaciarCarrito()" 
